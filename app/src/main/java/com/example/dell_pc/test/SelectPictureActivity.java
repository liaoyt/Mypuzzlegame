...
//    private String bmpDir;
    ...
    public static final String ACCOUNT_DIR = Environment.getExternalStorageDirectory().getPath()
            + "/PuzzleGame/";
    public static final String ACCOUNT_MAINTRANCE_PUZZLE_CACHE = "puzzle_cache/";
    private static final String IMGPATH = ACCOUNT_DIR + ACCOUNT_MAINTRANCE_PUZZLE_CACHE;

    private static final String IMAGE_FILE_NAME = "puzzleImage.jpeg";
    private static final String TMP_IMAGE_FILE_NAME = "tmp_puzzleImage.jpeg";
...
//                bmpDir="puzzle/pic"+(pic_num+1)+".jpg";
                ...
                Picasso.with(this).load(bmpID).into(iv);
                break;
            case 1:
                modes=1;
                getLocalPic();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == SELECT_A_PICTURE) {
            if (resultCode == RESULT_OK && null != data) {
                //				Log.i("zou", "4.4以下的");
                imageUri=Uri.fromFile(new File(IMGPATH, TMP_IMAGE_FILE_NAME));
                Bitmap bmp = null;
                try {
                ...
                if(bmp==null){
                    finish();
                    Intent intent = new Intent(SelectPictureActivity.this, PictureListActivity.class);
...
//			mAcountHeadIcon.setImageBitmap(bitmap);
         /*else if (requestCode == TAKE_A_PICTURE) {
            ...
            //			} else {  //低版本
            if (resultCode == RESULT_OK && null != data) {
                imageUri = Uri.fromFile(new File(IMGPATH, IMAGE_FILE_NAME));
                Bitmap bmp = null;
                try {
                    bmp = MediaStore.Images.Media.getBitmap(this.getContentResolver(), imageUri);
                } catch (IOException e) {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            case R.id.start:
                Intent intent = new Intent(SelectPictureActivity.this, piece.class);
